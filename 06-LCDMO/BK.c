...
//    Journal: 
//
// 1-27-17		06-LCDMO:  (LCD Display by Matrix Orbital)  This is a handy front panel interface
//						that I have used on a couple of projects.  It has a 4 x 20 LCD display, afew control
//						buttons, and 3 multi-color LEDs.  It's a pretty good front panel for simple projects.
//						We can use it as a front panel for a lot of tutorial projects.
//
//						It has RS232, so we can use the U2 stuff from previous projects.  This requires the 
//						following cable:
//						Signal		DM32-J11		Color		LCDMO
//						TX(out)		4						BRN			2			Data input
//						RX(in)		1						WHT			3			Data output
//						GND				5						BLK			4			GND 
//						+5V				J9-1				RED			1			+5V
//						
//						I have a library of LCDMO routines that make it easy to write stuff to the
//						display.  Check out the LCDMO routines.
//
// 1-25-17		05-U2MSG:  Adding in routine to send a message string out the serial port.
//            In C, a string is an array of ASCII characters terminated by a NULL (byte = 0).  The
...
{														// program will start here.
	...
	char CharRecd;						// Use this variable for characters received, define as "char" (8-bits)
  ...
  while (1)
  {
		...
		// Do this during startup - Allow contrast adjustment
		if ( CharRecd && StartUpTimer )					// ( StartUpTimer must be > 0. Interrupt counts it down from 5,000)
		{
			if ( CharRecd == 'B' )									// We get this character when up arrow is pressed
				LCDMO_ContrastUp();
			else if ( CharRecd == 'H' )						// We get this character when down arrow is pressed
				LCDMO_ContrastDown();
			else if ( CharRecd == 'E' )						// Terminate StartUp timing if center key pressed
				StartUpTimer = 0;
		...
		// Do this after startup, i.e., when StartUpTimer has decremented down to 0
		// IF startup is finished AND we have a character in the Rx buffer THEN
		// We can use this routine to respond to key presses...
		if ( !StartUpTimer && (U2_RxBufIn != U2_RxBufOut) )			// Only happens if StartUpTimer is 0 AND pointers not equal		
			U2_ProcKeyEntry();				// Process key entry (Routine will bump up pointer
